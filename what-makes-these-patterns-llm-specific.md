...
The recurring shape across all of these is that the gateway supplies a *mechanism* while the pattern owns the *intent, the domain join, and the decision the mechanism drives* — most sharply in [Per-User Consumption Budget](../per-user-consumption-budget/), where the gateway can throttle and cap but only the application knows which resource to bound and what the user should get when the budget is exhausted.
