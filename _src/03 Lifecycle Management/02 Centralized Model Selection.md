...
The fix was mechanical once found — swap to the live snapshot the rest of the codebase already used — but *once found* took a dig through production logs, and the swap had to be repeated at five sites. 
...
- Providers deprecate and retire dated snapshots on *their* schedule, so a hardcoded ID that works today returns a 404 at some future date with no change on your side. 
