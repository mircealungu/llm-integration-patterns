...
Simplified articles carry a similar state, arrived at differently. Each stored version has a `broken` flag that keeps it out of what learners are offered, and it is set three ways: an automatic quality check at generation time, an audit tool that finds versions the LLM wrote in the wrong language, and readers themselves, since three reports mark an article broken.[^cvt-broken]
...
[^cvt-broken]: `Article.broken`, `UserArticleBrokenReport`, and `available_simplified_versions` in the `zeeguu/api` repository.
